...
- 2026-09-16（群聊入口）：用户确认的计划——首页导航「联系 simon」改为「加入群聊」（锚点 #contact 不变）；联系区下方新增群聊小卡（标题 + 说明 + 「加微信拉你进群」按钮，复用 copy-button 交互复制 simonsxxs，加好友备注进群）。方案经用户选定：暂不放群二维码（微信群码 7 天过期），后续有稳定群码可升级为群码展示。教程页文字提及与 FAQ 文案未动。发布 run 35049852583，线上验证：导航改名、群聊卡渲染、按钮复制值、原联系卡与页脚均正常。
