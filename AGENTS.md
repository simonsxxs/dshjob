...
- 2026-09-16（页脚微调）：用户指正 dshgeo.com / opcmode.com 未上线——页脚两站改为纯文本（视觉与链接一致：同色同下划线、不可点），后接小字「正在快马加鞭开发中」；dshopc.com 保留跳转；check-site.py 关联站点白名单收窄为 dshopc.com（提交见 git log，run 35047000931，线上与截图已验证）。另核实 Cloudflare 自动邮箱混淆不影响联系区邮箱显示与复制（可见文本经脚本解码、data-copy 属性未改写）。
- 2026-09-16（纠错）：上一条「页脚微调」系对用户截图红框的误读（错把 Agent 卡的「查看教程」识别成页脚链接），已 revert（`e48d627`）恢复页脚三个关联链接原状；实际按用户指正修改 Agent 智能体卡「查看教程 →」：外观不变、取消跳转、下加小字「正在快马加鞭开发中」（run 35048009749，线上已验证 span 无 href、颜色同品牌蓝）。软件下载 / MCP 两张「开发中」卡片的「查看教程」暂保留可点（用户红框仅圈 Agent 卡，是否同步处理待用户示下）。
- 2026-09-16（红框需求正确执行）：经用户确认的计划——软件下载 / Agent 智能体 / MCP 工具连接三张开发中卡的「查看 Gitee」「查看 GitHub」改为同外观 span（aria-disabled，悬停态也中和），点击不再跳转；每卡按钮下加小字「正在快马加鞭开发中」（用户从两个候选措辞中选定）；Agent 卡「查看教程」恢复可点（撤销上轮误改）；Skill 卡按钮保持可点；页脚不动。发布 run 35048789643 因 CI 归属核对 grep -q 提前关管道致 curl 误报 exit 23 而失败（响应体增大后必然触发），以提交修复后 run 35048938839 成功；线上逐卡验证（SPAN 不可点/cursor auto/提示语在位/Skill 可点）+ 截图核对通过。
- 2026-09-16（群聊入口）：用户确认的计划——首页导航「联系 simon」改为「加入群聊」（锚点 #contact 不变）；进群方式经用户选定：点击「加入群聊」按钮复制微信号 simonsxxs、加好友后拉进群（暂不放 7 天过期的群二维码）。位置经用户第二次指正修正：**不做独立卡片**，按钮直接放在联系大卡左侧空白区（「一起把第一步走顺。」下方）；样式仅 .group-entry 一条外边距，复用 copy-button 交互。教程页文字与 FAQ 未动。发布 run 35050208476，线上验证：独立卡已删、按钮在位、复制值正确。
